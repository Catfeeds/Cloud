...
class HotelUserController extends ComController
{
    public function index()
    {
        ...
        $user = M('member');
        //$user = M('member')->where("org_id=".$org_id)->select();
        //dump($user);die;
        $pagesize = 10;#每页数量
        ...
        $list = $user->field("{$prefix}member.*,{$prefix}auth_group.id as gid,{$prefix}auth_group.title,{$prefix}org.org_name")
            ...
            ->where($where)
            ->where("org_id = $org_id and h_id>0")
    ...
    public function edit()
    {
        ...
        $org_id =  $_SESSION["org_id"];
        //dump($org_id);die;
        $userhotel = M('hotel')->field('id,h_name')->where("org_id = $org_id")->select();
        $this->assign('userhotel', $userhotel);
    ...
    public function update($ajax = '')
    {
        ...
        $hotel_id = isset($_POST['h_id']) ? intval($_POST['h_id']) : 0;
        if (!$hotel_id) {
            $this->error('请选择餐厅！');
        }
        //dump($hotel_id);die;
        ...
        //dump($data['org_id']);die;
        $data['h_id'] = isset($_POST['h_id']) ? trim($_POST['h_id']) : '';
        ...
        if (!$uid) {
            ...
            M('auth_group_access')->data(array('group_id' => 9, 'uid' => $uid))->add();
            ...
            if (M('member')->where("user='$user'")->count()&&$user!=$olduser) {
                $this->error('用户名已被占用！');
            }
            M('auth_group_access')->data(array('group_id' => 9))->where("uid=$uid")->save();
    ...
    public function add()
    {
        ...
        $userhotel = M('hotel')->field('id,h_name')->where("org_id = $org_id")->select();
        $this->assign('userhotel', $userhotel);
        $this->assign('org_id',$org_id)->display('form');
    }
}